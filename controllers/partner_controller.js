...
const partner = require(__dirname + "../../models/partner_model");
...
const get = async (req, res, next) => {
    res.json(await partner.get(req));
}

const getAll = async (req, res, next) => {
    res.json(await partner.getAll(req));
}

const update = async (req, res, next) => {
    if (await pm.modifyAdmin(req)) {
        res.json(await partner.update(req));
    } else {
...
const insert = async (req, res, next) => {
    if (await pm.modifyAdmin(req)) {
        res.json(await partner.insert(req));
    } else {
        res.json(new dbe.QueryResult([{}], "You don't have the permissions. ", 403))
    }
}
...
const deleted = async (req, res, next) => {
    if (await pm.modifyAdmin(req)) {
        res.json(await partner.deleted(req));
    } else {
        res.json(new dbe.QueryResult([{}], "You don't have the permissions. ", 403))
    }
}
